...
class AppEntryController extends Vue {
	...
	getWeChatOpenId() {
		...
		//请求微信接口获取openid
		// if (query.code) this.$store.commit("GET_OPENID", query.code);
		if(query.code){
			uni.request({
			    url: '/api/edu-wechat/wechat/user-info', //仅为示例，并非真实接口地址。
			    data: {
			        code: query.code
			    },
			    success: (res) => {
			        alert(JSON.stringify(res.data));
					this.$mRouter.push({route:this.$mRoutesConfig.weixin_bind})
			    },
				fail:(err)=>{
