<template>
		  ...
		  <movable-area class="drag-sort" :style="{height:boxHeight }" id="drag">
			...
			:class="{'active': active == index, 'vh-1px-t': item.index > 0}">
			  <view class="item text-df">
				  <view :class="['iconfont tfeduicon-pc','appicon']"></view>
				  <text style="font-size: 13px;" class="margin-top-xs text-gray">{{item[props.label]}}</text>
				  <view @click="close(index)" class="cuIcon-roundclosefill closeIcon text-gray"></view>
			  </view>
			</movable-view>
		  </movable-area>
	</view>
  ...
  methods: {
    onUpdateCurrentList (list = this.list) {
      ...
      for (const key in list) {
		  ...
		  let x = 0
		  if(key <= 3){
			  x = key * this.windowWidth * 0.25
		  }else{
			  if((Number(key)+1)%4 === 0){
				   x = 3 * this.windowWidth * 0.25
			  }else{
				  x = ((Number(key)+1)%4-1) * this.windowWidth * 0.25 
			  }
	...
	moveUpdateCurrentList(index){
      for (const i in this.currentList) {
		  ...
		  if(index == key){
			  ...
			  if(key <= 3){
				  this.currentList[i].x = key * this.windowWidth * 0.25
			  }else{
				  if((Number(key)+1)%4 === 0){
					   this.currentList[i].x = 3 * this.windowWidth * 0.25
				  }else{
					   this.currentList[i].x = ((Number(key)+1)%4-1) * this.windowWidth * 0.25 
				  }
			  } 
		  }
      }
