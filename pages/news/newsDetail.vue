<template>
	<view class="container">
		...
		<view class="tui-news-content">
			...
			<image src="../../static/img/news/banner_2.jpg" class="tui-article-pic" mode="widthFix"></image>
			<view class="tui-article">
			...
			<image src="../../static/img/news/banner_1.jpg" class="tui-article-pic" mode="widthFix"></image>
			<view class="tui-article">
		...
		<view class="tui-operate-box">
			...
			<button @tap="share" open-type="share" class="tui-share-btn">
		...
		<view class="tui-cmtbox">
			<view class="tui-cmt-cell" v-for="(item,index) in cmtList" :key="index">
				<image :src="'../../static/img/news/'+item.avatar" class="tui-avatar"></image>
...
<script>
	...
	import tuiBadge from "@/components/thor-ui/badge/badge"
	export default {
		components: {
			tuiIcon,
			tuiTag,
		...
		methods: {
			...
			btnCmt: function() {
				this.$mRouter.push({route:this.$mRoutesConfig.news_cmt})
				
			},
			cmtAll: function() {
				this.$mRouter.push({route:this.$mRoutesConfig.news_cmt_list})
			},
			cmtReply: function() {
				this.$mRouter.push({route:this.$mRoutesConfig.news_cmt_reply})
			}
		},
		// 页面上拉触底事件的处理函数
