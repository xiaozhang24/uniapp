		...
		<swiper indicator-dots autoplay circular :interval="5000" :duration="150" indicator-color="rgba(255, 255, 255, 0.9)"
		 indicator-active-color="#5677fc" class="tui-banner-swiper">
			<swiper-item v-for="(item,index) in banner" :key="index" @tap.stop="bannerDetail">
				...
				<image :src="'../../static/img/news/'+item.img" class="tui-slide-image" mode="widthFix" />
			</swiper-item>
		</swiper>
						...
						<view class="tui-news-picbox" :class="[item.isVideo || item.imgNum>1?'tui-w-full':'tui-w220 tui-h165',item.imgNum>1?'tui-flex-between':'']"
						 v-if="item.imgNum>0">
							<block v-for="(items,index2) in item.img" :key="index2">
								<image :src="'../../static/img/news/'+items" mode="widthFix" class="tui-block" :class="[item.imgNum>1?'tui-one-third':'',item.isVideo?'tui-w-full':'']"></image>
							</block>
...
<script>
	...
	export default {
		...
		methods: {
			search: function() {
				this.$mRouter.push({route:this.$mRoutesConfig.news_search})
			},
			bannerDetail: function() {
				this.$mRouter.push({route:this.$mRoutesConfig.newsDetail})
			},
			detail(e) {
				let index = e.index;
				let url = this.$mRoutesConfig.newsDetail;
				if (this.newsList[index].isVideo) {
					url = this.$mRoutesConfig.news_video;
				}
