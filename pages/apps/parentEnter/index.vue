	...
	<view class="leave">
		<view style="margin-bottom: 100rpx;">
			<view v-if="tabIndex==0">
				<view class="padding text-gray">请选择进校类型</view>
				<view class="cu-list menu">
					...
					<view class="cu-item arrow" @tap="toFormCar">
						<view class="content">
						...
						<tui-list-cell @click="toLeaveDetail(item)" padding="0">
							<view class="tui-goods-item solid-bottom">
								...
								<view class="tui-goods-center margin-left-xs">
									<view class="tui-goods-name text-lg text-bold">{{item.applyUser}}提交的进校申请</view>
									<view class="tui-goods-attr">申请类型：家长进校</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			toForm() {
				uni.navigateTo({
					url: '/pages/apps/parentEnter/form?appId='+this.appId
				})
			},
			toFormCar(){
				uni.navigateTo({
					url: '/pages/apps/parentEnter/form_car?appId='+this.appId
				})
			},
			//获取请假记录
			getRecords() {
				this.$refs.loading.open()
				records({createUser:this.userInfo.user_id}).then(res => {
