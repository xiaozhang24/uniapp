	...
	<view class="bg-white">
		<view class="tui-searchbox bg-gray">
			<view class="tui-search-input margin-left margin-right">
				...
				<!-- #endif -->
				<input confirm-type="search" placeholder="请输入教师姓名"  placeholder-class="tui-input-plholder"
			...
			<view @tap="toDetail(item)" class="cu-item arrow" v-for="(item,index) in teacherList" :key="index">
				<view class="content padding-top-xs padding-bottom-xs">
					...
					<view class="text-gray text-sm">
						...
						<text>{{item.politics==1?'党员':'团员'}}</text>
					</view>
				</view>
				<view class="action">
				</view>
		...
		<!--加载loadding-->
		<tui-fab v-if="permission.teacherRecord_add" :left="0" :right="80" :bottom="100" :bgColor="bgColor"  @click="addTeacher"></tui-fab>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			toDetail(item) {
				uni.navigateTo({
					url: "/pages/apps/teacherRecord/detail?id=" + item.id
				})
			},
			...
			addTeacher(){
				uni.navigateTo({
					url:"/pages/apps/teacherRecord/add"
				})
			}
		}
	}
