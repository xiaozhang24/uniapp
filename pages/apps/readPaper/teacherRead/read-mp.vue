<template>
	<view class="read">
		...
		<view class="tui-flex tui-align-between text-center bg-nRed padding-xs">
			<text @tap="showQuestionModel" class="tui-col-4">题干</text>
			<text @tap="standardArea" class="tui-col-4">参考答案</text>
			<text @tap="toggleQues" class="tui-col-4">选题</text>
			<text @tap="toggleBoard" class="tui-col-4">打分板<text class="cuIcon-order"></text></text>
			<text @tap="showStudentList" class="tui-col-4">回评</text>
		</view>
		<!-- 标题栏和状态栏占位符 -->

		<!-- <view class="titleNview-placing"></view> -->

		<view class="padding-top-xs tui-flex padding-bottom-xs padding-left-lg padding-right-lg text-sm solid-bottom">
			<text class="tui-col-4">进度 {{readProgress}}/{{formatQuesList.length}}</text>
			<text class="tui-col-4">满分：{{formatQuesList[questionIndex].score}}</text>
			<text class="tui-col-4">总分：{{totalScore}}</text>
			<text class="tui-col-4">得分：{{formatQuesList[questionIndex].currentScore!=null?formatQuesList[questionIndex].currentScore:'未评'}}</text>
		</view>
		<!-- 试卷展示区域 -->
		
		<view class="padding paperArea">
			<!-- <text class="text-lg">考生答案：</text> -->
			<image class="margin-top" @tap="previewImg" mode="widthFix" v-for="item in questionList[questionIndex].answerArea"
			 :src="item.img"></image>
		</view>

		<view v-if="commentShow" class="commentArea-APP bg-gray padding-xs tui-flex-box animation-slide-down">
			...
			<view class="tui-flex tui-align-between">
				...
				<view @tap="keyInput" class="tui-col-4 item item2" data-score="3">3</view>
			...
			<view class="tui-flex tui-align-between">
				...
				<view @tap="keyInput" class="tui-col-4 item item2" data-score="9">9</view>
		...
		<view class="cu-modal" :class="questionShow?'show':''">
			<view class="cu-dialog">
				...
				<view class="cu-bar bg-white">
					<view class="action margin-0 flex-sub  solid-left" @tap="hideQuestionModel">我知道了</view>
				</view>
			</view>
		</view>
		<!-- 考生预览 -->
			...
			<view style="overflow-y: scroll;" class="cu-dialog basis-lg" @tap.stop="" :style="[{top:CustomBar+'px',height:'calc(100vh - ' + CustomBar + 'px)'}]">
				<view class="cu-list menu text-left">
					...
					<view class="padding-xs">
						<text style="width: 140rpx;display: inline-block;">学生序号</text>
						<text style="width:160rpx;display: inline-block;">已阅/全部</text>
						<text style="display: inline-block;">得分</text>
					</view>
					<view @tap="chooseStudent(item.student)" class="cu-item arrow" v-for="(item,index) in studentList" :key="index">
						<view class="content">
							<text style="width: 140rpx;display: inline-block;">{{index+1}}</text>
							<text style=" width:160rpx;display: inline-block;">{{item.status.length}}/{{questionList.length}}</text>
							<text style="display: inline-block;">{{item.totalScore}}</text>
						</view>
					</view>
				</view>
			</view>
		...
		<view class="cu-modal drawer-modal justify-start" :class="questionModel?'show':''" @tap="hideQuestion">
			<view style="overflow-y: scroll;" class="cu-dialog basis-lg" @tap.stop="" :style="[{top:CustomBar+'px',height:'calc(100vh - ' + CustomBar + 'px)'}]">
				<view class="cu-list menu text-left padding-xs">
					<view class="padding-left">
						<text style="width: 100rpx;display: inline-block;">题号</text>
						<text style="display: inline-block;">评分情况</text>
						<text class="padding-left" style="display: inline-block;">得分</text>
					</view>
					<view @tap="chooseQues(index)" class="cu-item arrow" v-for="(item,index) in formatQuesList" :key="index">
						<view class="content">
							<text style="width: 100rpx;display: inline-block;">{{item.label}}</text>
							<text style="width: 160rpx;display: inline-block;">{{item.currentScore!=null?'已评':'未评'}}</text>
							<text style="display: inline-block;">{{item.currentScore}}</text>
						</view>
					</view>
				</view>
			</view>
						...
						data.forEach(item => {
							if (item.id == groupId) {
								...
								item.questionList.forEach(ques => {
									ques.label = ques.maxTopic + "-" + ques.minTopic
